...
DEBEZIUM_CONFIG=$(cat "$CONFIG_DIR/connectors/mysql-source.json")
...
if [ -f "$CONFIG_DIR/connectors/clickhouse-sink.json" ]; then
    CLICKHOUSE_CONFIG=$(cat "$CONFIG_DIR/connectors/clickhouse-sink.json")
else
    print_error "clickhouse-sink.json not found in $CONFIG_DIR/connectors/"
